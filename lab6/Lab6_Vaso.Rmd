...
GA <- function(maxiter,mutprob){
  ...
  while (i <= maxiter){
      ...
      victim <- order(values)[1] #index of the smallest obj func value 
...
At first we are asked to plot function f in the range from 0 to 30 and see if there's a maximum value. So, in this graph we can observe the global maximum value is approximately 0.20 and is given when x is around 2.
...
For **maxiter**=10 we can see that the algorithm does not converge to the global maximum for none of the three probabilities. When *mutprob*=0.5 the max value is -1.549543 which is the biggest compared to the other two max values obtained with mutation probabilities 0.1 and 0.9 but still it's quite far from the global maximum. We would say that probably 10 iterations were not enough for the algorithm to converge and the algorithm gets stuck to local maxima.

On the other hand, when **maxiter**=100 we see that the algorithm converged with *mutprob*=0.5 and the maximum value obtained is 0.234853. For *mutprob*=0.9 the maximum value found was -0.3782303 which was close to the global but still not converged and the maximum value obtained with *mutprob*=0.1 was even further from the optimal one.

To conclude,the algorithm found the optimal maximum value(0.234853) for **maxiter**=100 and *mutprob*=0.5.
...
First thing we observe when looking at this time series plot is that there Z has some blank points which means that some data are missing. Z has quite a few peaks, with the highest ones being observed for small values of X. Y also has a quite similar pattern to Z, with peaks that are more intense at the beginning, but generally smaller values than Z.
...
$l(\lambda|Y,Z)$ = $-2nlog(\lambda)$ - $\frac{\sum_{i=1}^{n} x_i y_i}{\lambda}$ - $\frac{\sum_{i=1}^{n} x_i z_i}{2\lambda}$ + $C$
...
$E[l(\lambda|Y,Z)]$ = $E[$ $-2nlog(\lambda)$ - $\frac{\sum_{i=1}^{n} x_i y_i}{\lambda}$ - $\frac{\sum_{i \in observed} x_i z_i}{2\lambda}$ - $\frac{\sum_{j \in unobserved} x_j z_j}{2\lambda}$ + $C$ $]$

 All terms except $\frac{\sum_{j \in unobserved} x_j z_j}{2\lambda}$ are from observed data. Expected value of a constant number is a constant number. So there is only one term that involves uncertainty. Since `Z` follows exponential distribution with parameter $\frac{x_j}{2\lambda_{t-1}}$, unobzerved $Z_i$'s expected value will be $\frac{2\lambda_{t-1}}{x_j}$, such that ${\lambda_{t-1}}$ represents $\lambda$ value obatained in previous iteration.
 
$E[$ $\frac{\sum_{j \in unobserved} x_j z_j}{2\lambda}$ $]$
= $\sum_{j \in unobserved} \frac{x_j}{2\lambda} \frac{2\lambda_{t-1}}{x_j}$
= $\sum_{j \in unobserved} \frac{\lambda_{t-1}}{\lambda}$
= $U$ $\frac{\lambda_{t-1}}{\lambda}$, U is a number of unobserved `Z`.

5. Take a derivative of the expected loglikelihood with respect to $\lambda$

$\frac{\partial l(\lambda)}{\partial \lambda}$ = $\frac {-2n}{\lambda}$ + $\frac{\sum_{i=1}^{n} x_i y_i}{\lambda^2}$ + $\frac{\sum_{i \in observed} x_i z_i}{2\lambda^2}$ + $U$ $\frac{\lambda_{t-1}}{\lambda^2}$

6. Set above equal to 0, find $\lambda$ and update its value

$\lambda$ = $\frac{(\sum_{i=1}^{n} x_i y_i)+(\frac{\sum_{i \in observed} x_i z_i}{2})+(U\lambda_{t-1})}{2n}$
