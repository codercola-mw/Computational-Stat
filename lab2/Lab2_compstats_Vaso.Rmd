...
```{r}
RNGversion("3.5.1")
data <- read.csv2("C:/Users/Vaso/Desktop/data computational/mortality_rate.csv")
...
```{r}
data2 <- load("C:/Users/Vaso/Desktop/data computational/data.RData",verbose = T)
```
...
$0=\frac{\partial l(x_{1},...,x_{n}|\theta)}{\partial \sigma} \\= - \frac{n}{\sigma} + \frac{1}{\sigma^3}\sum_{i=1}^{n}(x_{i}-\mu)^2 \\ \Rightarrow - \frac{n}{\sigma} + \frac{1}{\sigma^3}\sum_{i=1}^{n}(x_{i}-\mu)^2=0 \\ \Rightarrow \sum_{i=1}^{n}(x_{i}-\mu)^2 = n\sigma^2 \\ \Rightarrow \hat{\sigma^2}= \frac{1}{n}\sum_{i=1}^{n}(x_{i}-\mu)^2 $
...
log_likelihood <- function(mu,sigma,data) {
  n <- 100
  loglik <-(-n*log(2*pi) - n*log(sigma^2)- 1/sigma^2*sum(data-mu)^2 )/2 
  return(-loglik)
}

mu_mle <- sum(data)/length(data)
sigma_mle <- sum(data-mean(data))^2/length(data)
 
```


```{r}
#cg <- optim(c(0,1),log_likelihood,NULL,method="CG")
```
